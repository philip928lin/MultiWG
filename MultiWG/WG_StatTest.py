...
def SpatialAutoCorrelationComparison(Setting, Stat, Wth_gen):
    ...
    for v in Var:
        ...
        SaveFig(fig, Setting["MultiSite"]["SpatialAutoCorrIndex"] + "_" + v,
                Setting)
    return None
...
def KSTestCDFPlot(Stat_Stn, Stn, Setting):
    ...
    for i in range(3):
        for j in range(4):
            ...
            axs[j,i].plot(x, expon(coef1[0],coef1[1]).cdf(x), label='exp',
                          linestyle=':' )
            axs[j,i].plot(x, gamma(coef2[0],coef2[1],coef2[2]).cdf(x),
                          label='gamma', linestyle=':' )
            axs[j,i].plot(x, weibull_min(coef3[0],coef3[1],coef3[2]).cdf(x),
                          label='weibull', linestyle=':' )
            xlog = np.arange(min(np.log(Prep_m)),max(np.log(Prep_m)),0.1)
            axs[j,i].plot(np.exp(xlog), norm(coef4[0],coef4[1]).cdf(xlog),
                          label='lognorm', linestyle=':' )
            axs[j,i].plot(ecdf.x, ecdf.y, label='ecdf', color = "red")
            axs[j,i].axvline(x = 130, color = "black", linestyle = "--",
...
def PdistTest(Stat_Stn, Method = "kstest", alpha = 0.05):
    ...
    for m in range(0,12):
        ...
        if Method == "kstest":            
            exp = KSTest(Prep_m, expon(coef1[0],coef1[1]).cdf, alpha,
                         "exp "+str(m+1))
            gam = KSTest(Prep_m, gamma(coef2[0],coef2[1],coef2[2]).cdf, alpha,
                         "gamma "+str(m+1))
            wei = KSTest(Prep_m, weibull_min(coef3[0],coef3[1],coef3[2]).cdf,
                         alpha, "weibull "+str(m+1))
            logn = KSTest(np.log(Prep_m), norm(coef4[0],coef4[1]).cdf, alpha,
                          "lognorm "+str(m+1))
            ...
            #Ppearson3.append(KSTest(Prep_m,pearson3(
                # coef5[0],coef5[1],coef5[2]).cdf))           
    data = {"exp":Pexpon,
            "gamma":Pgamma,
            "weibull":Pweibull,
            "lognorm":Plognorm}
    StatPdistTest = pd.DataFrame(data, columns = data.keys(),
                                 index = np.arange(1,13) )
    data2 = {"exp":Pexpon2,
            "gamma":Pgamma2,
            "weibull":Pweibull2,
            "lognorm":Plognorm2}
    StatPdistPvalue = pd.DataFrame(data2, columns = data.keys(),
                                   index = np.arange(1,13) )
...
def PdistBIC(Stat_Stn):
    ...
    for m in range(0,12):
        ...
        coef3 = MonthlyStat.loc[m+1,"weibull"]
        coef4 = MonthlyStat.loc[m+1,"lognorm"]
        Pexpon.append(BIC(n, 1, np.sum(expon.logpdf(Prep_m,
                                                    coef1[0],coef1[1])) )) 
        Pgamma.append(BIC(n, 1, np.sum(gamma.logpdf(Prep_m, coef2[0],coef2[1],
                                                    coef2[2])) )) 
        Pweibull.append(BIC(n, 1, np.sum(weibull_min.logpdf(Prep_m,
                                                            coef3[0],
                                                            coef3[1],
                                                            coef3[2])) )) 
        Plognorm.append(BIC(n, 1, np.sum(norm.logpdf(np.log(Prep_m),
                                                     coef4[0],
                                                     coef4[1])) ))      
    data = {"exp":Pexpon,
            "gamma":Pgamma,
            "weibull":Pweibull,
            "lognorm":Plognorm}
    StatPdistBIC = pd.DataFrame(data, columns = data.keys(),
                                index = np.arange(1,13) )
    return StatPdistBIC
...
def WGFTest(Wth_gen, Wth_obv, Setting):
    ...
    for s in Stns:
        ...
        FtestDict[s]["pvalue"] = pd.DataFrame(
            f.cdf(FtestDict[s]["Statistic"], Dof[s]["Obv"], Dof[s]["Gen"]),
            columns = Var, index = np.arange(1,13))
        # Turn into result based on alpha
        FtestDict[s]["TestResult"] = pd.DataFrame(
            index = FtestDict[s]["pvalue"].index,
            columns = FtestDict[s]["pvalue"].columns).fillna("Pass")
...
def WGTTest(Wth_gen, Wth_obv, Setting):
    ...
    for s in Stns:
        ...
        Count[s]["Dof"] = (Std[s]["Obv"]**2/Count[s]["Obv"] \
            + Std[s]["Gen"]**2/Count[s]["Gen"])**2\
            / ((Std[s]["Obv"]**2/Count[s]["Obv"])**2/(Count[s]["Obv"]-1)\
                +(Std[s]["Gen"]**2/Count[s]["Gen"])**2/(Count[s]["Gen"]-1))
        Std[s]["Std_TTest"] =  (Std[s]["Obv"]**2/Count[s]["Obv"] \
            + Std[s]["Gen"]**2/Count[s]["Gen"])**0.5       
        TtestDict[s]["Statistic"] = np.abs((Avg[s]["Obv"]-Avg[s]["Gen"])\
            / Std[s]["Std_TTest"]) # Two tails
        TtestDict[s]["pvalue"] = pd.DataFrame(
            t.cdf(TtestDict[s]["Statistic"], Count[s]["Dof"]),
            columns = Var, index = np.arange(1,13))
        # Turn into result based on alpha
        TtestDict[s]["TestResult"] = pd.DataFrame(
            index = TtestDict[s]["pvalue"].index,
            columns = TtestDict[s]["pvalue"].columns).fillna("Pass")
...
def MonthlyStatPlot(Wth_gen, Wth_obv, Setting):
    ...
    for s in Stns:
        ...
        fig.text(0.05, 0.5, 'Generated', va='center', rotation='vertical',
                 fontsize = 14)
        fig.set_size_inches(18.5, 10.5)
        # rect : tuple (left, bottom, right, top)
        plt.tight_layout(rect=[0.06, 0.05, 0.94, 0.94]) 
...
def NormalTest(residual, Stat_s, Setting):
    ...
    for v in Var:      
        ...
        if res[1] < 0.05:  
            restxt = "Reject"
            Stat_s["Warning"].append(
                "NormalTest: Normal test is rejected at "+v)
        else:               
            restxt = "Pass"

Setting_template = {"WDPath": "Working directory",    
           ...
           "WthObvCsvFile": None,  
           #(Default None, == StnID.csv. {"StnID": "filename.csv"})
           "ClimScenCsvFile":None, 
           #(Default None, which no parameters will be updated. 
           # {"StnID":"filename.csv"})
           "Var": ["PP01", "TX01", "TX02", "TX04"],
           #(Weather variables: Using Taiwan's Central Weather Bureau 
           # standard code.)
           "P_Threshold": 0.01,                     
           ...
           # Options: "exp", "gamma", "weibull", "lognorm".)
           "GenYear": 200,                          
           #(Total generation years. If leapYear = True, 
           # it has to be a multiple of 4.)
           "Condition": True,                       
           #(Default True for ensuring the order of Tmin & Tmax and Tavg
           # are correct. 
           # If False, the order of generated data (Tmin & Tmax and 
           # Tavg) needs to be checked afterward.)
           "LeapYear": True,                        
           ...
           "FourierOrder": 2,
           #(Defualt 2. The order of the fourier fitting lines for 
           # non-precipitation
           # variables. Value can be 2 or 3 or 4.)
           "DumpCheck": True,
           #(Default True. Check the order of min max mean tempertature 
           # are right if Condition is set False. Check other non T or P
           # variable. If it < 0 then we interpolate it with values of 
           # index -2~+2.)
